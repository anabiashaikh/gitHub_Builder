...
export function TemplateSelector() {
  ...
  const templates = [
    ...
    },
    {
      ...
      gradient: "from-purple-900 to-slate-800",
    },
  ];
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-12">
      <header className="border-b border-outline-variant pb-4">
        <h2 className="font-sans text-2xl font-bold text-on-surface tracking-tight flex items-center gap-2">
          <Palette className="text-primary" /> Template & Theme Selector
        </h2>
        <p className="text-on-surface-variant font-sans text-xs mt-1">
          Choose a visual theme template for your GitHub profile header and banner pipeline.
        </p>
      </header>
        ...
        {templates.map((tpl) => {
          ...
          return (
            <div
              ...
              className={`p-5 rounded-lg border cursor-pointer transition-all duration-200 relative overflow-hidden flex flex-col justify-between ${
                isSelected
            ...
            >
              <div>
                <div
                  className={`h-16 rounded-md bg-gradient-to-r ${tpl.gradient} mb-4 relative overflow-hidden flex items-end p-2`}
                >
                  <div className="text-[10px] font-mono bg-black/40 text-white px-2 py-0.5 rounded backdrop-blur-sm">
                    {tpl.id}
                  </div>
                </div>
